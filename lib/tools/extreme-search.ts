...
class ExaContentStrategy implements ContentExtractionStrategy {
  ...
  async getContents(links: string[]): Promise<SearchResult[]> {
    ...
    // Use plain HTTP fallback for failed URLs
    if (failedUrls.length > 0) {
      console.log(`[Exa] Using HTTP fallback for ${failedUrls.length} URLs:`, failedUrls);

      for (const url of failedUrls) {
        try {
          const response = await fetch(url, { headers: { 'User-Agent': 'scira-extreme/1.0' } });
          if (!response.ok) {
            continue;
          }
          ...
          results.push({
            title: url.split('/').pop() || 'Retrieved Content',
            url,
            content: text,
            publishedDate: '',
          ...
          console.error(`[Exa] HTTP fallback error for ${url}:`, httpError);
        }
      }
    }
...
class ParallelContentStrategy implements ContentExtractionStrategy {
  ...
  async getContents(links: string[]): Promise<SearchResult[]> {
    ...
    // Use HTTP fallback for failed URLs
    if (failedUrls.length > 0) {
      console.log(`[Parallel] Using HTTP fallback for ${failedUrls.length} URLs:`, failedUrls);
      ...
      for (const url of failedUrls) {
        try {
          ...
          console.error(`[Parallel] HTTP fallback error for ${url}:`, httpError);
        }
      }
    }
...
async function extremeSearch(
  ...
  const parallel = contentProvider === 'parallel' ? new Parallel({ apiKey: serverEnv.PARALLEL_API_KEY }) : null;

  // Create search and content extraction strategies based on provider
  const searchStrategy: SearchProviderStrategy =
    contentProvider === 'parallel' && parallel
      ? new ParallelSearchStrategyForExtreme(parallel)
      : new ExaSearchStrategy(exa);

  const contentStrategy: ContentExtractionStrategy =
    contentProvider === 'parallel' && parallel ? new ParallelContentStrategy(parallel) : new ExaContentStrategy(exa);
