...
This repository ships a self-contained `docker-compose.yml`:
- No `build` step in compose (uses GHCR image)
- No `.env` file required (all variables are in the compose file)

1. Make sure Docker and Docker Compose are installed
2. Edit `docker-compose.yml` and replace the placeholder secrets (for example `EXA_API_KEY`, `BETTER_AUTH_SECRET`)
3. Run:
   ```bash
   docker compose up -d
   ```
